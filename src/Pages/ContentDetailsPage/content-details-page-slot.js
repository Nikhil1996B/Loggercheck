...
import { ContentDetailsProvider } from "./ContentDetailsSlot/contentdetailsconsumer";
import Contentdetailspagelayout from "./contentdetailspagelayout";
...
const availableContentDetailsPageComponent = [
  ...
  },
  {
    ...
    component: ContentDetailsProvider,
  },
  {
...
function ContentDetailsPageContent({ user }) {
  // Initial setup
  ...
  useEffect(() => {
    getByGenrer("Action").then((res) => {
      ...
      dispatch({ type: "CONTENT_PAGE_CONTENT", ...{ payload: res } });
    });
  ...
  return (
    <main className={`${themeName}-content-details-Page`}>
...
export default ContentDetailsPageContent;
